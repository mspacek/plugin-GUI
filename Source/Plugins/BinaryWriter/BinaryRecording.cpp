...
void BinaryRecording::openMessageFile(String basepath, int recordingNumber)
{
    ...
    if (!fileExists)
    {
        String header = generateMessageHeader();
        fwrite(header.toUTF8(), 1, header.getNumBytesAsUTF8(), f);
        fflush(f); // ensure header is written to disk immediately
    }
    else
...
String BinaryRecording::generateMessageHeader()
{
    String header = "## Generated by Open-Ephys, Busse Lab\n";
    header += "version = '" + String(GITVERSION) + "'\n";
    header += "## Recording metadata:\n";
    header += "datetime = " + generateDateString() + "\n";
    //TODO: improve datetime formatting
    header += "sampfreq = " + String(getChannel(0)->sampleRate) + " # Hz\n";
    header += "AD2uV = " + String(getChannel(0)->bitVolts) + "\n";
    //TODO: what does bitVolts actually represent?
    //TODO: electrode layout, or at least design
    //TODO: nchannels
    header += "## Message log format: samplei <SPACE> message\n";
    header += "## sample0 corresponds to first sample in .dat file\n";
    return header;
}
